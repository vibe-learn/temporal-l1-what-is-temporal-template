...
func TestTaskQueueDefault(t *testing.T) {
	if os.Getenv("TEMPORAL_TASK_QUEUE") == "" && TaskQueue() != "lesson-l1_what_is_temporal-tq" {
		t.Errorf("TaskQueue() default = %q, want lesson-l1_what_is_temporal-tq", TaskQueue())
	}
}
...
func TestOrderWorkflow(t *testing.T) {
	...
	env.OnActivity(SendEmail, mock.Anything).Return("ok", nil)
...
func TestIntegration(t *testing.T) {
	...
	// поведения урока «Что такое Temporal».
}
